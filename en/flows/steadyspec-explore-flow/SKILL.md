...
Read `.steadyspec/substrate.json` if present. If missing or `lastAdopt` field absent: surface "no adopt baseline recorded" as INFORMATIONAL in the attention report and recommend the user run `steadyspec-adopt` once. Do not block.

v0.3 treats adopt signals as routing input, not automatic governance escalation. If a baseline exists, report material drift such as LOC growth, change count growth, repeated module drift, or a new structural class as needs-glance unless it creates a hard high-risk trigger.
