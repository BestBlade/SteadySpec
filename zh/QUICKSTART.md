...
`auto` 也只能处理配置允许的低风险切片。范围变化、需求缩减、公共/安全/数据等高风险语义、proof 策略变化、证据缺口、环境失败、无法收敛和残余未知仍归人。`candidate-ready` 不是接受、合并、归档、发布或真理。当前发布边界是 Windows 单用户；源代码树验证不能替代最终发布前的全新 packed-install smoke。
