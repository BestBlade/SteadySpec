...
### 反漂移方法，以及一份实验性的 Agent Assurance 协议

长时间跟 AI Agent 协作，有一个安静的失败模式：Agent 慢慢改了你的
意图，决策丢了归属，验证被当成了真相，最终记录被清理到不再描述
实际发生过什么。v0.7 把此前混在一起的内容拆成四层：

1. [METHOD.md](METHOD.md) 解释为什么会漂移，以及责任归谁；
2. [v0.7 Assurance Protocol Candidate](../protocol/ASSURANCE_PROTOCOL.md)
   约束一个进程能对哪一份精确 target / candidate / evidence /
   assessment 声称什么；
3. implementation / adapter 把协议接到运行时；v0.7 只有一个无依赖参考进程；
4. recipe / profile 规定如何工作；五阶段软件 SDD 与 v0.6
   Critic/Builder/Evaluator closure 是保留的 legacy recipe。

`ready-for-human` 是协议最强的声明：当前精确快照可以在已记录覆盖范围
内交给人做检查。它不是语义真理、人的接受、审查独立性，也不是合并、
归档、发布、部署或风险接受授权。

不安装技能、不编辑 closure policy，也可以先运行：
...
这是保留的 legacy 软件 recipe，不是 v0.7 协议的规范来源。
...
- **独立 Assurance 进程：** `assurance` 实现实验性协议候选；不安装、
  不调用五个软件动词也能单独评估。
- **Legacy recipe 辅助 CLI：** `init`、docs `check`、`cross-review`、
...
发生不兼容变化，并必须记录在 [CHANGELOG.md](../CHANGELOG.md)。以下 legacy
软件 recipe 表面打算保持稳定，除非 changelog 明确说打破：
